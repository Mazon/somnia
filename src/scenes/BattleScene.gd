...
func prepare_battle(player_team, enemy_team):
	...
	for combatant in player_team.combatants:
		...
		#$UserInterface/PlayerHealthBars.add_child(new_health_bar)
	...
	for combatant in enemy_team.combatants:
		...
		new_combatant.connect("status_changed", self, "status_changed", [new_health_bar])
		new_combatant.connect("buffs_changed", self, "buffs_changed", [new_health_bar])
		#$UserInterface/EnemyHealthBars.add_child(new_health_bar)
...
func start_turn():
	...
	turn_count += 1
	$UserInterface/TurnCounter.text = str("Turn: ", turn_count)
...
func show_combatant_actions(current_combatant):
	...
	$UserInterface/Actions.add_child(pass_action_button)
...
func show_action_targets(acting_combatant, action):
	...
	match(action.target):
		...
		Constants.ActionTarget_EnemySingle:
			for combatant in $Combatants/Ysort.get_children():
				if combatant.player_id != acting_combatant.player_id:
					...
					$UserInterface/Targets.add_child(new_target)
		...
		Constants.ActionTarget_EnemyMultiple:
			...
			$UserInterface/Targets.add_child(new_multi_target_button)
...
func erase_actions_ui():
	for target_button in $UserInterface/Actions.get_children():
		target_button.queue_free()

# Clear the target buttons
func erase_targets_ui():
	for target_button in $UserInterface/Targets.get_children():
		target_button.queue_free()
...
func create_damage_label(damage, starting_position, color):
	...
	$UserInterface/Effects.add_child(new_damage_effect)

# Display some text using a cool tween to move the text around
func show_text(text):
	$UserInterface/Text/EnemyAction.text = text
	
	$UserInterface/Text/EnemyAction/Tween.interpolate_property($UserInterface/Text/EnemyAction, "rect_global_position", 
			$UserInterface/Text/EnemyAction.rect_global_position, Vector2(0, 100), 0.5)
	$UserInterface/Text/EnemyAction/Tween.start()

# Removes the text from the screen
func reset_text():
	$UserInterface/Text/EnemyAction.rect_global_position = Vector2(0, -100)
...
func create_effect(starting_position, effect_reference):
	...
	$UserInterface/Effects.add_child(new_effect)

# Show a combatant stats on the screen
func update_target_info(combatant):
	$UserInterface/TargetInfo/Name.text = combatant.combatant_name
	$UserInterface/TargetInfo/HPValue.text = str(combatant.current_hp, " / ", combatant.max_hp)
	$UserInterface/TargetInfo/ProtValue.text = str(combatant.protection)
	$UserInterface/TargetInfo/AtkValue.text = str(combatant.min_attack, " - ", combatant.max_attack)
	$UserInterface/TargetInfo/SpeedValue.text = str(combatant.speed)

# Removes the currrent combatant stats on the screen
func clear_target_info():
	$UserInterface/TargetInfo/Name.text = "//"
	$UserInterface/TargetInfo/HPValue.text = "//"
	$UserInterface/TargetInfo/ProtValue.text = "//"
	$UserInterface/TargetInfo/AtkValue.text = "//"
	$UserInterface/TargetInfo/SpeedValue.text = "//"

# Makes one unique target as if it was hovered
func focus_target_hover(current_target):
	for target in $UserInterface/Targets.get_children():
...
func remove_combatant_ui(combatant):
	for health_bar in $UserInterface/PlayerHealthBars.get_children():
		if health_bar.combatant == combatant:
			health_bar.queue_free()
	for health_bar in $UserInterface/EnemyHealthBars.get_children():
		if health_bar.combatant == combatant:
			health_bar.queue_free()
